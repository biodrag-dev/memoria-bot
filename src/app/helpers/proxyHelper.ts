...
export async function proxyCheck(
  ...
  if (proxy === null) {
    ...
    if (proxy[0] != "Narrator" && !message.reference) {
      ...
      characterHelper.addCurrency(
        message.client,
        id,
        proxy[0],
        Math.max(wordCount - debt, 0),
      );
    }
    if (message.reference) {
      ...
      if (original.author.username == proxy[1].nick) {
        if (proxy[0] != "Narrator") {
          ...
          const debt: number = proxyDex[id].proxies[proxy[0]]!.expDebt;
          proxyDex[id].proxies[proxy[0]]!.expDebt = Math.max(
            debt - (newWordCount - oldWordCount),
            0,
          );
          await characterHelper.addCurrency(
            message.client,
            id,
            proxy[0],
            Math.max(newWordCount - oldWordCount - debt, 0),
          );
          saveProxies();
        }
...
export function getAllProxyNames(userID: string) {
  ...
  for (const [character, proxyData] of Object.entries(
...
export async function editProxyDetails(
...
export function proxyDelete(userID: string, message: Message) {
  ...
  if (proxy) {
    const wordCount: number = message.content.trim().split(/\s+/).length;
...
export async function sendWebhook(
  ...
  await webhook.edit({
    channel: message.channel.isThread()
...
export function balanceEmbed(id: string, name: string) {
  ...
  const embed = new EmbedBuilder();
  embed.setAuthor({
    name: `${name}'s Bank Account`,
    iconURL: proxyDex[id]?.proxies[name]?.character.pfp_link,
  });
